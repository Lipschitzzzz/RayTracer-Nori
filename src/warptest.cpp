...
static const std::string kWarpTypeNames[WarpTypeCount] = {
    ...
    "cosine_hemisphere", "phong_specular", "beckmann", "microfacet_brdf", "phong_brdf"
};
...
struct WarpTest {
    ...
    std::pair<bool, std::string> run() {
        ...
        auto integrand = [&](double y, double x) -> double {
            if (warpType == Square) {
                ...
                else if (warpType == PhongSpecular)
                    return Warp::squareToPhongSpecularPdf(v, parameterValue);
    ...
    std::pair<Point3f, float> warpPoint(const Point2f &sample) {
        ...
        switch (warpType) {
            ...
            case PhongSpecular:
                result << Warp::squareToPhongSpecular(sample, parameterValue); break;
            case Beckmann:
    ...
    create_phong_bsdf(float kd, float exponent, float percent_diffuse, float bsdfAngle) {
        ...
        list.setColor("specular_albedo", Color3f(kd));
        list.setFloat("percent_diffuse", percent_diffuse);
        list.setFloat("exponent", exponent);

        auto * brdf = (BSDF *) NoriObjectFactory::createInstance("phong", list);
...
public:
    ...
    static float mapParameter(WarpType warpType, float parameterValue) {
        ...
        if (warpType == PhongSpecular || warpType == PhongBRDF)
            parameterValue *= 100;
        return parameterValue;
    }
    static float mapParameter2(WarpType warpType, float parameterValue) {
        if (warpType == Beckmann || warpType == MicrofacetBRDF)
            parameterValue = std::exp(std::log(0.01f) * (1 - parameterValue) +
                                      std::log(1.f)   *  parameterValue);
        return parameterValue;
    ...
    void refresh() {
        ...
        float parameter2Value = mapParameter2(warpType, m_parameter2Slider->value());
        ...
        if (!m_brdfValueCheckBox->checked() || !(warpType == MicrofacetBRDF || warpType == PhongBRDF))
            value_scale = 0.f;
        ...
        m_parameterSlider->set_enabled(warpType == Beckmann || warpType == PhongSpecular || warpType == MicrofacetBRDF || warpType == PhongBRDF);
        m_parameterBox->set_enabled(warpType == Beckmann || warpType == PhongSpecular || warpType == MicrofacetBRDF || warpType == PhongBRDF);
        m_parameter2Slider->set_enabled(warpType == MicrofacetBRDF || warpType == PhongBRDF);
        m_parameter2Box->set_enabled(warpType == MicrofacetBRDF || warpType == PhongBRDF);
        m_angleBox->set_enabled(warpType == MicrofacetBRDF || warpType == PhongBRDF);
        m_angleSlider->set_enabled(warpType == MicrofacetBRDF || warpType == PhongBRDF);
        m_brdfValueCheckBox->set_enabled(warpType == MicrofacetBRDF || warpType == PhongBRDF);
    ...
    void draw_contents() {
        ...
        if (m_drawHistogram) {
            ...
            if (m_warpTypeBox->selected_index() == MicrofacetBRDF || m_warpTypeBox->selected_index() == PhongBRDF) {
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox = new ComboBox(m_window, { "Square", "Tent", "Disk", "Sphere", "Hemisphere (unif.)",
                "Hemisphere (cos)", "Phong specular", "Beckmann distr.", "Microfacet BRDF", "Phong BRDF" });
