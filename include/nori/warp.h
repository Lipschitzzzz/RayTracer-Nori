...
public:
    ...
    /// Phong specular weighted sample a vector on the unit hemisphere around the pole (0,0,1) with respect to projected solid angles
    static Vector3f squareToPhongSpecular(Point2f sample, float exponent);

    /// Probability density of \ref squareToPhongSpecular()
    static float squareToPhongSpecularPdf(const Vector3f &v, float exponent);
